...
public class Main {
	...
	public static void main(String[] args) {
		...
		final File file = new File(inputDirectoryPath);
		for (final File f : loadFiles(file)) {
		...
		for (final Table t : Constants.tables) {
			if ((t != null) && (t.getFields() != null)
					&& (t.getFields().size() > 0)) {
		...
		if (generated.size() > 0) {
			final String[] data = generated
					.toArray(new String[generated.size()]);
		...
		System.out.println("TOTAL Constants.tables = "
				+ Constants.tables.size());
		for (final Table t : Constants.tables) {
	 ...
	 *            contains "variants"
	 */
	public static void parseJsonObject(JSONObject obj, String currentObjName,
			String exObjName, boolean isInArray) {
		...
		if ((currentObjName != null) && !currentObjName.isEmpty()
				&& !Utils.tableAlreadyExists(currentObjName)) {
			currentTable = new Table(currentObjName,
					Utils.getNameCamelCase(currentObjName), null, null,
					isInArray);
			Constants.tables.add(currentTable);
		}

		final Iterator iter = obj.entrySet().iterator();
		while (iter.hasNext()) {
			...
			if ((exObjName != null) && !exObjAlreadySet) {
				createField(exObjName + "FkId", currentObjName,
						Constants.JUNCTION, exObjName, aObj);
			...
			if (aObj instanceof JSONObject) {
				...
				final JSONArray array = (JSONArray) aObj;
				System.out
						.println("IN " + currentObjName + " # ARRAY : " + key);
				if (array.size() > 0) {
					final Object bObj = array.get(0);

					if (bObj instanceof JSONObject) {
						parseJsonObject((JSONObject) bObj, key, currentObjName,
								true);
					} else {
						System.out.println("++++ parseJsonObject/ARRAY key = "
								+ key + " aObj = " + aObj + " currentObjName "
								+ currentObjName);
						createField(key, currentObjName, Constants.ARRAY, null,
								bObj);
					}
				}
